...
export default function CustomApp() {
  ...
  return (
    <section className="py-24 relative overflow-hidden bg-gradient-to-br from-white via-purple-50/10 to-indigo-50/15 min-h-screen">
      {/* Background visual accents */}
      <div className="absolute top-1/4 left-10 w-96 h-96 rounded-full bg-purple-100/20 blur-3xl -z-10 animate-pulse-slow"></div>
      <div className="absolute bottom-10 right-10 w-96 h-96 rounded-full bg-indigo-100/20 blur-3xl -z-10"></div>

      <div className="max-w-7xl mx-auto px-6 relative">
        {/* Header Block */}
        <div className="text-center space-y-6 max-w-4xl mx-auto mb-16">
          <ScrollReveal delay={100}>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100/60 shadow-sm">
        ...
        <ScrollReveal delay={400} className="mb-20">
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 max-w-3xl mx-auto bg-white/70 backdrop-blur-md border border-slate-200/50 rounded-3xl p-6 shadow-md">
            {/* Price tag block */}
            <div className="bg-indigo-50/50 border border-indigo-100/80 rounded-2xl p-4 px-6 text-center md:text-left flex flex-col justify-center min-w-[200px]">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-750 text-white font-semibold py-4.5 px-8 rounded-2xl shadow-lg shadow-indigo-100 hover:shadow-xl hover:shadow-indigo-200 transition-all duration-300 w-full md:w-auto text-sm"
            >
            ...
            <button
              ...
              className="py-4.5 px-6 rounded-2xl border border-slate-200 hover:border-slate-350 hover:bg-slate-50 text-slate-650 hover:text-slate-900 font-semibold transition-all duration-300 w-full md:w-auto text-sm"
            >
              Compare All Plans
            </button>
          </div>
              ...
              <ScrollReveal key={idx} delay={idx * 100} className="h-full">
                <div
                  className={`relative rounded-3xl p-8 pt-10 transition-all duration-300 hover:scale-[1.02] flex flex-col justify-between text-left h-full group/card overflow-hidden ${
                    cap.highlight
                      ? 'border-2 border-indigo-600/35 bg-white shadow-xl shadow-indigo-50/50 ring-4 ring-indigo-600/5'
                      : 'border border-slate-200/80 bg-white/50 backdrop-blur-md hover:bg-white/80 hover:shadow-lg'
                  }`}
                >
                  {/* Accent border at top */}
                  <div className={`absolute top-0 left-0 right-0 h-[3px] ${
                    cap.highlight ? 'bg-indigo-600' : 'bg-slate-200 group-hover/card:bg-indigo-500'
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="relative w-12 h-12">
                      ...
                      <div className={`absolute inset-0 rounded-2xl border scale-95 opacity-40 transition-all duration-300 group-hover/card:-rotate-6 ${
                        cap.highlight ? 'border-purple-400' : 'border-indigo-200'
                      }`} style={{ transform: 'translate(4px, 4px)' }}></div>
                    </div>

                    {/* Titles & Description */}
                    <div className="space-y-3">
                      <h3 className="text-lg font-bold text-slate-850 tracking-tight transition-colors group-hover/card:text-indigo-600">
                        {cap.title}
                      </h3>
